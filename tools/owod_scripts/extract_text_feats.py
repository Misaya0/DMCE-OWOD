...
def extract_feats(model, dataset=None, task=None, save_path='embeddings'):
    ...
    text_feats = model.backbone.forward_text([prompt]).squeeze(0).detach().cpu() #通过文本编码器提取每个类别的文本特征
...
def extract_tuned_feats(config=None, ckpt=None, wildcard='object', save_path="embeddings"):
    ...
    model_path = 'work_dirs/yolo_uniow_s_lora_object_ft_bn_1e-4_3e_8gpus_owod/best_lvis_AR@100_epoch_5.pth'
...
if __name__ == "__main__":
    ...
    if args.extract_tuned:
        ...
        checkpoint = runner.load_checkpoint(args.ckpt, map_location='cpu')

...
class YOLOv10Head(BaseDenseHead):
    ...
    def __init__(self,
        ...
        self.use_SPGA = self.head_module.use_SPGA
    ...
    # 添加一致性正则化
    def consistency_loss(self, cls_preds,anchor_scores, unknown_mask):
        pred_object_native = cls_preds[:, :, -2].sigmoid()
        return F.mse_loss(pred_object_native[unknown_mask], anchor_scores[unknown_mask])
    ...
    def one2many_loss_by_feat(
        ...
        current_featmap_sizes = [ #每个尺度特征图的尺寸
        ...
        if current_featmap_sizes != self.featmap_sizes_train:
            ...
            mlvl_priors_with_stride = self.prior_generator.grid_priors(#每个特征图位置的先验框
        ...
        gt_labels = gt_info[:, :, :1]#真实标签
        gt_bboxes = gt_info[:, :, 1:]  # 真实边界框（xyxy）
        pad_bbox_flag = (gt_bboxes.sum(-1, keepdim=True) > 0).float()#填充标志，1为有效边界框，0为填充边界框
        ...
        if self.anchor_label and self.num_classes < 80:
            ...
            anchor_scores = flatten_cls_preds[:, :, -1].detach().sigmoid()#微调后的object得分，覆盖所有真实框，如果得分比较高，则隐含了“这可能是一个物体”的语义信息
            max_known_scores = flatten_cls_preds[:, :, :-2].detach().sigmoid().amax(dim=2)#最大已知类别的分类分数
            ...
            # consistency_loss = self.consistency_loss(flatten_cls_preds, anchor_scores, unknown_mask)
            # weight = torch.sigmoid((anchor_scores - 0.5) * 10)  # 高分样本获得更高权重
        ...
        # max_indices[max_indices == self.num_classes+1] = 20
        tmp_labels = max_indices
        assigned_scores_sum = assigned_scores.sum().clamp(min=1)
        if self.num_classes < 80:
            loss_cls = self.loss_cls(flatten_cls_preds, assigned_scores).sum()
        else:
            loss_cls = self.loss_cls(flatten_cls_preds[:, :, :-2], assigned_scores[:, :, :-2]).sum()
        ...
        return dict(
            ...
            # one2many_consistency_loss=consistency_loss * num_imgs * world_size,
        ), tmp_labels
    ...
    def one2one_loss_by_feat(
        ...
        if self.anchor_label and self.num_classes < 80:
            ...
            # consistency_loss = self.consistency_loss(flatten_cls_preds, anchor_scores, unknown_mask) * 0.1
        ...
        return dict(
            ...
            # one2one_consistency_loss=consistency_loss * num_imgs * world_size,
        ),tmp_labels
    ...
    def predict_by_feat(self,
        ...
        if self.anchor_label:
            ...
            # flatten_cls_scores[:, :, -2] *= 2 # 调整未知类得分
        ...
        for (bboxes, scores, objectness,
            ...
            results = InstanceData(
                scores=scores, labels=labels,cosinescores=cosine[keep_idxs],
                        logits=logits[keep_idxs], bboxes=bboxes[keep_idxs])
            ...
            if cfg.get('unknown_nms', None):
                # ori_max_per_img的值会影响召回率
                ori_max_per_img = cfg.max_per_img
                # ori_max_per_img = 1000
                cfg.max_per_img = len(results)
            ...
            if self.use_SPGA:
                ood_threshold = 3
                unknown_index = self.num_classes - 2

                ood_score = -results.cosinescores[:,:unknown_index].max(dim=1).values
                results.ood_score = ood_score
                is_ood = (ood_score > ood_threshold)
                # is_ood = (ood_score > ood_threshold) | (results.scores < 0.1)
                results.labels[is_ood] = unknown_index
            # ---------- 👆 你加入的 OOD 判断逻辑z ----------

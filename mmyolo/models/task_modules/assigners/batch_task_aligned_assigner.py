...
class BatchTaskAlignedAssigner(nn.Module):
    ...
    def forward(
        ...
        priors = priors[:, :2]# 仅保留先验框中心坐标 (cx, cy)
        ...
        (assigned_gt_idxs, fg_mask_pre_prior,
         pos_mask) = select_highest_overlaps(pos_mask, overlaps, num_gt)#选择最佳匹配的gt

        # assigned target
        assigned_labels, assigned_bboxes, assigned_scores = self.get_targets(#根据标签分配结果生成目标标签、边界框和分类分数，用于计算分类和回归损失。
            gt_labels, gt_bboxes, assigned_gt_idxs, fg_mask_pre_prior,
            batch_size, num_gt)

        # normalize
        alignment_metrics *= pos_mask# 过滤无效候选
        pos_align_metrics = alignment_metrics.max(axis=-1, keepdim=True)[0]# 每个预测框的最大对齐指标
        pos_overlaps = (overlaps * pos_mask).max(axis=-1, keepdim=True)[0]# 每个预测框的最大IoU
        norm_align_metric = (
            alignment_metrics * pos_overlaps /
            (pos_align_metrics + self.eps)).max(-2)[0].unsqueeze(-1)
        assigned_scores = assigned_scores * norm_align_metric#综合对齐指标和重叠度，调整分类分数权重
        ...
        assigned_result['pos_mask'] = pos_mask.bool()
        return assigned_result
    ...
    def get_pos_mask(self, pred_bboxes: Tensor, pred_scores: Tensor,
        ...
        alignment_metrics, overlaps = \
            self.get_box_metrics(pred_bboxes, pred_scores, gt_labels,
                                 gt_bboxes, batch_size, num_gt)#计算预测框与真实框的对齐指标和重叠度

        # get is_in_gts mask
        is_in_gts = select_candidates_in_gts(priors, gt_bboxes)#确保先验框中心点落在gt的范围内
        ...
        pos_mask = topk_metric * is_in_gts * pad_bbox_flag#属于前k个候选、中心在gt范围内、pad_bbox_flag为1的bbox

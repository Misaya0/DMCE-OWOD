...
class RandomLoadText:
    ...
    def __call__(self, results: dict) -> dict:
        ...
        negative_labels = random.sample(
            candidate_neg_labels, k=min(len(candidate_neg_labels), num_neg_samples))
...
@TRANSFORMS.register_module()
class ClassAgnosticLabel:

    def __call__(self, results: dict) -> dict:
        results['gt_bboxes_labels'] = np.array([0 for _ in results['gt_bboxes_labels']])
        return results